...
<?php
	...
	$total_pages_sql = "SELECT Count( * ) AS count FROM hm_fwban WHERE ban_reason LIKE '%{$search}%' AND flag=1";
	$result = mysqli_query($con,$total_pages_sql);
	$total_rows = mysqli_fetch_array($result)[0];
	$total_pages = ceil($total_rows / $no_of_records_per_page);

	$sql = "SELECT id, DATE_FORMAT(timestamp, '%y/%m/%d %H:%i.%s') as TimeStamp, ipaddress, ban_reason, countrycode, country, flag FROM hm_fwban WHERE ban_reason LIKE '%{$search}%' AND flag=1 ORDER BY TimeStamp DESC LIMIT $offset, $no_of_records_per_page";
...
echo number_format($total_rows)." IP".$singular." banned for reason <b>".$search."</b> have been released (Page: ".number_format($page)." of ".number_format($total_pages).")<br />";
echo "<table class='section'>
	...
	if($row['flag'] === NULL) echo "<td>No</td>";
	else echo "<td>YES</td>";
...
<div class="section">
RS = Released Status (removal from firewall)<br />
</div>
